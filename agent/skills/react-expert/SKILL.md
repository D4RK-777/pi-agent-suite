...
0. **Load project + house context FIRST (non-negotiable)** — before writing code:
   - `mempalace_search(<task description>, wing="konekt_nextjs", n=5)` — existing code in the **Gloss** project (wing name is a legacy identifier; product is Gloss). Components, hooks, and types from this wing override anything generic.
   - `mempalace_search(<task description>, wing="expert-knowledge", n=3)` — curated patterns and decisions from the Obsidian vault.
   - If the task mentions a specific component, also check `obsidian_search` in `patterns/` and `decisions/`.
   - Skip this step only if the `<mempalace-context>` block auto-injected into the prompt already fully covers the task (rare for non-trivial work).
   House rules from `%USERPROFILE%.claude\CLAUDE.md` always win over any generic React advice below (design tokens, no Inter, no purple gradients, WCAG 2.2 AA, TS strict).
