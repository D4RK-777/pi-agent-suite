...
0. **Load project + house context FIRST (non-negotiable)** — before writing code:
   - `mempalace_search(<task description>, wing="konekt_nextjs", n=5)` — the **Gloss** Next.js app's existing routes, server actions, layouts (wing name is a legacy identifier; product is Gloss). Match its App Router conventions exactly.
   - `mempalace_search(<task description>, wing="expert-knowledge", n=3)` — curated patterns and decisions from the Obsidian vault.
   - If the task references a specific route or API, also check `obsidian_search` in `patterns/` and `decisions/`.
   - Skip only if auto-injected `<mempalace-context>` already covers it.
   House rules (design tokens, no Inter, no purple gradients, WCAG 2.2 AA, TS strict) from `%USERPROFILE%.claude\CLAUDE.md` always win.
