...
0. **Load project + house context FIRST (non-negotiable)** — before introducing types or generics:
   - `mempalace_search(<task description>, wing="konekt_nextjs", n=5)` — existing types, interfaces, branded types, tsconfig settings from the **Gloss** project (wing name is a legacy identifier). Reuse before inventing.
   - `mempalace_search(<task description>, wing="expert-knowledge", n=3)` — curated type patterns and decisions from the vault.
   - House rule: TS strict, no `any`. Never widen types to make errors go away.
